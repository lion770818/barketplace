...
func (tk *ClientData) CreateAuth(userid uint64, td *TokenDetails) error {
	...
	atCreated, err := tk.client.Set(context.TODO(), td.TokenUuid, strconv.Itoa(int(userid)), at.Sub(now)).Result()
	if err != nil {
		return err
	}
	rtCreated, err := tk.client.Set(context.TODO(), td.RefreshUuid, strconv.Itoa(int(userid)), rt.Sub(now)).Result()
	if err != nil {
		return err
	}
...
func (tk *ClientData) FetchAuth(tokenUuid string) (uint64, error) {
	userid, err := tk.client.Get(context.TODO(), tokenUuid).Result()
	if err != nil {
		return 0, err
	}
...
func (tk *ClientData) DeleteTokens(authD *AccessDetails) error {
	...
	//delete access token
	deletedAt, err := tk.client.Del(context.TODO(), authD.TokenUuid).Result()
	if err != nil {
		return err
	}
	//delete refresh token
	deletedRt, err := tk.client.Del(context.TODO(), refreshUuid).Result()
	if err != nil {
		return err
	}
...
func (tk *ClientData) DeleteRefresh(refreshUuid string) error {
	//delete refresh token
	deleted, err := tk.client.Del(context.TODO(), refreshUuid).Result()
